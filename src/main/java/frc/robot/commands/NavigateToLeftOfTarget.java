...
public class NavigateToLeftOfTarget extends Command {
  ...
  public void execute() {
    ...
    try {
      ...
      // Calculate target position (to the left of the AprilTag, accounting for stopping distance)
      // Left side is +Y in field coordinates (forward is +X)
      Translation2d tagTranslation = tagPose.getTranslation();
      Translation2d targetOffset = new Translation2d(0, kOffsetDistanceMeters); // Left side (positive Y)
      ...
      if (distanceToTag > kStoppingDistanceMeters) {
        ...
        // This ensures we maintain the left offset while stopping short
      ...
      // Debug: Log tag and target positions
      System.out.println(String.format(
          "[NavigateToLeft] Tag at (%.2f, %.2f), Target at (%.2f, %.2f), Robot at (%.2f, %.2f)",
          tagTranslation.getX(), tagTranslation.getY(),
          finalTargetPosition.getX(), finalTargetPosition.getY(),
      ...
      if (needsNewPath && m_targetPose != null) {
        try {
          ...
          System.out.println("[NavigateToLeft] Generated PathPlanner path to target at (" +
              String.format("%.2f", m_targetPose.getX()) + ", " +
              String.format("%.2f", m_targetPose.getY()) + ")");
        } catch (Exception e) {
          System.out.println("[NavigateToLeft] Error generating path: " + e.getMessage());
          e.printStackTrace();
          m_currentPath = null;
        }
      }
      ...
      if (m_currentPath != null && m_targetPose != null) {
        ...
        // Debug output
        System.out.println(String.format(
            "[NavigateToLeft] robot=(%.2f,%.2f,%.1f°) target=(%.2f,%.2f) " +
        ...
        SmartDashboard.putNumber("NavigateToLeft/Distance to Target", distanceToTarget);
        SmartDashboard.putNumber("NavigateToLeft/X Error", xError);
        SmartDashboard.putNumber("NavigateToLeft/Y Error", yError);
        SmartDashboard.putNumber("NavigateToLeft/Heading Error (deg)", Math.toDegrees(headingError));
        SmartDashboard.putNumber("NavigateToLeft/Angle to Target (deg)", Math.toDegrees(angleToTarget));
        SmartDashboard.putNumber("NavigateToLeft/Current Heading (deg)", Math.toDegrees(currentHeading));
      } else {
        // No path, stop
        m_drivetrain.arcadeDrive(0.0, 0.0);
      }
    } catch (Exception e) {
      System.out.println("[NavigateToLeftOfTarget] Error: " + e.getMessage());
      e.printStackTrace();
      m_drivetrain.arcadeDrive(0.0, 0.0);
    }
  }
